...
namespace Unity.Entities.SourceGen.SystemGeneratorCommon
{
    public static class PartialSystemTypeGenerator
    {
            ...
            Generate(SystemDescription[] allDescriptionsForTheSameSystem)
        {
            ...
            generatedPartialType = AddOnCreateForCompilerWithFields(generatedPartialType, allDescriptionsForTheSameSystem, description.Compilation, description.SystemType == SystemType.ISystem);
        ...
        private static TypeDeclarationSyntax AddOnCreateForCompilerWithFields(TypeDeclarationSyntax generatedPartialType, SystemDescription[] descriptions, Compilation compilation, bool isISystem)
        {
            var allQueryFieldsToGenerate =
                descriptions.SelectMany(d=> d.QueryFieldsToFieldNames).ToArray();

            foreach (var fieldDescription in allQueryFieldsToGenerate)
                generatedPartialType = generatedPartialType.AddMembers(fieldDescription.Key.GetFieldDeclarationSyntax(fieldDescription.Value));

            var allNonQueryFieldsToGenerate = descriptions.SelectMany(d=>d.NonQueryFields).Distinct().ToArray();
            foreach (var fieldDescription in allNonQueryFieldsToGenerate)
                generatedPartialType = generatedPartialType.AddMembers(fieldDescription.FieldDeclaration);

            var additionalSyntax = descriptions.SelectMany(d => d.AdditionalStatementsInOnCreateForCompilerMethod).Distinct().ToArray();

            if (isISystem || generatedPartialType.Members.Any()) // Only needs to create OnCreateForCompiler in SystemBase if members are present
                generatedPartialType =
                    generatedPartialType.AddMembers(
                        EntitiesSourceFactory.OnCreateForCompilerMethod(
                            additionalSyntax,
                            allNonQueryFieldsToGenerate,
                            allQueryFieldsToGenerate,
                            GetAccessModifiers(compilation), isISystem));
        ...
        private static TypeDeclarationSyntax CreatePartialSystemType(SystemDescription[] descriptions, TypeDeclarationSyntax originalSyntax)
        {
            ...
            switch (originalSyntax)
            {
                case StructDeclarationSyntax _:
                    ...
                    allFullyQualifiedBaseTypeNames = allFullyQualifiedBaseTypeNames.Append("Unity.Entities.ISystemCompilerGenerated");
                    break;
                default:
            ...
            */
            ...
            for (var i = 0; i < systemDescriptions.Count; i++)
            {
                ...
                if (!description.Rewriters.Any())
                    continue;
                var originalFilePath = description.SyntaxTreeInfo.Tree.FilePath;
        ...
        static (SyntaxTokenList, SyntaxList<AttributeListSyntax>) GetModifiersAndAttributes(string targetMethodNameAndSignature, MemberDeclarationSyntax rewritten, bool isMethod)
        {
            ...
            var attributeName = isMethod ? "Unity.Entities.DOTSCompilerPatchedMethod" : "Unity.Entities.DOTSCompilerPatchedProperty";
        ...
        static string GetAccessModifiers(Compilation compilation)
        {
