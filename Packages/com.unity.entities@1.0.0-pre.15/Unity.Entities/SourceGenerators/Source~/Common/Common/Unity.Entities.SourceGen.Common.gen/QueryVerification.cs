    ...
    public static partial class SourceGenHelpers
    {
        public static class QueryVerification
        {
                ...
                string invokedMethodName)
            {
                ...
                foreach (var query in queries)
                {
                    ...
                    bool isValidQueryType =
                        ...
                        queryTypeSymbol.InheritsFromInterface($"Unity.Entities.IAspect<{queryTypeSymbol.ToFullName()}>") ||
                ...
                bool compareTypeSymbolsOnly = false)
            {
                ...
                {
                    if (query.TypeSymbol.ToFullName().StartsWith("Unity.Entities.Ref") &&
                ...
                var conflictingQueryTypes = mutuallyExclusiveQueryTypes1.Intersect(mutuallyExclusiveQueryTypes2).ToArray();

                foreach (var conflict in conflictingQueryTypes)
                    QueryConstructionErrors.SGQC004(systemDescription, location, queryGroup1Name, queryGroup2Name, conflict.ToFullName());
