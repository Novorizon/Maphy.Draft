...
using VerifyCS = Unity.Entities.Analyzer.Test.CSharpCodeFixVerifier<
    Unity.Entities.Analyzer.EntitiesAnalyzer,
...
namespace Unity.Entities.Analyzer
{
    ...
    public class EntitiesAnalyzerUnitTest
    {
        ...
        public async Task StoreBlobAssetReferenceValueInLocal()
        {
            ...
            ";
            var expected = VerifyCS.Diagnostic(EntitiesAnalyzer.ID_EA0001).WithLocation(0)
        ...
        public async Task WorksFromFieldErrors()
        {
            ...
            var expected = VerifyCS.Diagnostic(EntitiesAnalyzer.ID_EA0001).WithLocation(0).WithArguments("root.Keys", "var", "keys");
            await VerifyCS.VerifyCodeFixAsync(test, expected, fixedSource);
        }
        ...
        public async Task ObjectInitOnNewVarErrors()
        {
            ...
            var expected = VerifyCS.Diagnostic(EntitiesAnalyzer.ID_EA0002).WithLocation(0).WithArguments("new Node()");
        ...
        public async Task PointerToFixedSize()
        {
            ...
            ";
            var expected = VerifyCS.Diagnostic(EntitiesAnalyzer.ID_EA0003).WithLocation(0).WithArguments("TestStruct256bytes", "TestStruct256bytes.array", "is a pointer.  Only non-reference types are allowed in Blobs.");
            await VerifyCS.VerifyAnalyzerAsync(test, expected);
        }
        ...
        public async Task WithPointerInBlob()
        {
            ...
            ";
            var expected = VerifyCS.Diagnostic(EntitiesAnalyzer.ID_EA0003).WithLocation(0)
